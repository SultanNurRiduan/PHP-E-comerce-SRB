...
$categoryType = 'sneakers'; // atau 'clothes' / 'aksesoris' tergantung halaman ini
...
<body class="bg-gray-100">
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      const searchInput = document.getElementById('search-input'); // dari navbar, bisa null
      ...
      function fetchFilteredProducts() {
        ...
        params.append('category_type', 'sneakers');

        const keyword = (searchInput?.value || "<?= htmlspecialchars($keyword) ?>").trim();
        if (keyword) params.append('q', keyword);

        fetch("/shoe-shop/app/product/fetch.php?" + params.toString())
          .then(res => res.text())
          .then(html => {
      ...
      // Update saat filter checkbox diubah
      checkboxes.forEach(cb => cb.addEventListener('change', fetchFilteredProducts));

      // Update saat user mengetik keyword di search input (jika tersedia)
